...
path= '/run/media/acooke/Elements/flux_tower_data/2018/'
...
results_2018 <- read.csv("/run/media/acooke/Elements/data/OSU/GEK/Flux_tower/concat_raw_2018/results_final/eddypro_GEK_full_output_2019-06-26T184725.csv",  skip=2, stringsAsFactors = F)
head(results_2018)


# results_2018 <- read.csv("/run/media/acooke/Elements/data/OSU/GEK/Flux_tower/2018_results_june/eddypro_GEK_full_output_2019-06-18T183512.csv",  skip=2, stringsAsFactors = F)
# head(results_2018)

# plot(results_2018[,40]*24, ylim=c(-5, 5))


data_readj <- data.frame(matrix(0, nrow=nrow(results_2018)+1, ncol=14))
# data_readj
...
# data_readj$Year <- 
data_readj$DoY[2:nrow(data_readj)] <- as.numeric(round(results_2018[,4]))

data_readj$NEE[2:nrow(data_readj)] <- as.numeric(results_2018[,14])
data_readj$qcNEE[2:nrow(data_readj)] <- as.numeric(results_2018[,15])
data_readj$LE[2:nrow(data_readj)] <- as.numeric(results_2018[,12])
data_readj$qcLE[2:nrow(data_readj)] <- as.numeric(results_2018[,13])
data_readj$H[2:nrow(data_readj)] <- as.numeric(results_2018[,10])
data_readj$qcH[2:nrow(data_readj)] <- as.numeric(results_2018[,11])
data_readj$rH[2:nrow(data_readj)] <- as.numeric(results_2018[,45])
data_readj$Ustar[2:nrow(data_readj)] <- as.numeric(results_2018[,60])
data_readj$VPD[2:nrow(data_readj)] <- as.numeric(results_2018[,46]/100) # transform Pa into hPa

data_readj$Tair[2:nrow(data_readj)] <- as.numeric(results_2018[,35])-273.15
# data_readj$Rg[2:nrow(data_readj)] <- "NA"
data_readj$Tsoil[2:nrow(data_readj)] <- "NA"

data_readj$Year[2:nrow(data_readj)] <- as.numeric(2018)
data_readj$Hour[2:nrow(data_readj)] <- as.numeric(substr(as.character(results_2018[,3]), 1,2))
...
write.table(data_readj, "/home/acooke/codes/pflotran/flux_tower_data/2018/2018_new.txt", row.names = F)
...
EddyData.F <- fLoadTXTIntoDataframe("/home/acooke/codes/pflotran/flux_tower_data/2018/2018_new.txt")
EddyData.F
...
seqdt<- seq(
  from=as.POSIXct("2018-01-01 0","%Y-%m-%d %H"),
  to=as.POSIXct("2018-12-31 23", "%Y-%m-%d %H"),
  by="30 min"
)

filling2018 <- data.frame(matrix(NA, nrow=length(seqdt)+1, ncol=12))
filling2018[1,] <- c("-", "-", "-","-", "umolm-2s-1",	"Wm-2",	"Wm-2",		"degC",	"degC",	"%",	"hPa", "ms-1")
colnames(filling2018) <- c("DateTime","Year", "DoY", "Hour", "NEE", "LE","H",  "Tair", "Tsoil", "rH", "VPD", "Ustar")

head(filling2018)
filling2018$Year[2:nrow(filling2018)] <- as.numeric(2018)
filling2018$DateTime[2:nrow(filling2018)] <- as.character(seqdt)
filling2018$Hour[2:nrow(filling2018)] <- round(as.numeric((format(strptime(filling2018$DateTime[2:nrow(filling2018)],"%Y-%m-%d %H"),'%H'))))
filling2018$Hour[seq(3, length(filling2018$Hour), 2)] <- as.numeric(filling2018$Hour[seq(3, length(filling2018$Hour), 2)])+0.5  
filling2018$DoY[2:nrow(filling2018)] <-round(as.numeric((format(strptime(filling2018$DateTime[2:nrow(filling2018)],"%Y-%m-%d %H"),'%j'))))
filling2018$Year[2:nrow(filling2018)] <- as.numeric(2018)
head(filling2018)

filling2018[1,]<- c("-", "-", "-", "-", "umolm-2s-1","Wm-2",	"Wm-2",		"degC",	"degC",	"%",	"hPa", "ms-1")

write.table(filling2018, "/home/acooke/codes/pflotran/flux_tower_data/2018/filling2018_new.txt", row.names = F)


filling2018.F <- fLoadTXTIntoDataframe("/home/acooke/codes/pflotran/flux_tower_data/2018/filling2018_new.txt")

filling2018.F$Hour <- as.character(filling2018.F$Hour)

filling2018.F$Hour <- as.numeric(gsub("," ,".", filling2018.F$Hour))


filling2018.F <- fConvertTimeToPosix(filling2018.F,'YDH',Year = 'Year'
                                     ,Day = 'DoY',Hour = 'Hour')
head(filling2018.F)

filling2018.F <- filling2018.F[,-2]
...
ts.tmp = rbind(EddyDataWithPosix.F , filling2018.F)
# by="DateTime", all=F
...
# 
# seqfix<- seq(
#   from=as.POSIXct("2018-12-31 00:30:00","%Y-%m-%d %H"),
#   to=as.POSIXct("2018-12-31 22:00:00", "%Y-%m-%d %H"),
#   by="30 min"
# )
...
# rep2018 <- rep(2018, length(seqfix))
...
EddyProc.C <- sEddyProc$new('2018',my_data1, 
...
plot(ET_daily$Date,ET_daily$ETp, type="b", ylab="ETa [mm]", xlab="2018", main="Flux tower after Gapfilling")
abline(h=0, col="red")
...
plot(ET_daily1$Date,ET_daily1$ETp, type="l", ylab="ETa [mm]", xlab="2018", main="Flux tower after/before Gapfilling", ylim=c(-0.15, 0.4))
abline(h=0, col="red")
